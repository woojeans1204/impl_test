...
PROJECT_REL_PATH = "impl_test/test6"
PROJECT_FULL_PATH = os.path.join(BASE_DIR, PROJECT_REL_PATH)

CONFIG_DIR = "configs/exp260218"
LOG_BASE_DIR = os.path.join(BASE_DIR, "test6/logs")
...
def worker(worker_id, task_queue, finished_count, lock, worker_status):
    # [수정된 부분] -------------------------------------------------------
    # 워커당 할당할 GPU 개수 및 ID 계산
    # 예: NUM_GPUS=4, NUM_WORKERS=1 -> gpus_per_worker=4 (GPU 0,1,2,3 모두 사용)
    # 예: NUM_GPUS=4, NUM_WORKERS=2 -> gpus_per_worker=2 (Worker0: 0,1 / Worker1: 2,3)
    gpus_per_worker = NUM_GPUS // NUM_WORKERS
    
    # 0번 워커는 0부터, 1번 워커는 gpus_per_worker부터 시작
    start_gpu_idx = worker_id * gpus_per_worker
    target_gpu_ids = [str(i) for i in range(start_gpu_idx, start_gpu_idx + gpus_per_worker)]
    
    # "0,1,2,3" 형태의 문자열 생성
    gpu_string = ",".join(target_gpu_ids)
    # -------------------------------------------------------------------
    ...
    while True:
        ...
        print(f"▶️ [Worker {worker_id} / GPUs {gpu_string}] 시작: {config_file}") # 이 줄이 있는지 확인!
        log_name = os.path.basename(config_file)
        log_path = os.path.join(LOG_BASE_DIR, f"log_{log_name}.out")

        # [수정 2] 총 에포크 읽어서 공유 딕셔너리에 저장
        full_conf_path = os.path.join(PROJECT_FULL_PATH, CONFIG_DIR, config_file)
        ...
        # [수정된 부분] -------------------------------------------------------
        # --num_processes를 할당된 GPU 개수(gpus_per_worker)만큼 설정
        cmd = (
            f"CUDA_VISIBLE_DEVICES={gpu_string} "
            f"accelerate launch --num_processes {gpus_per_worker} "
        ...
        send_slack_msg(f"✅ [Worker {worker_id}] 실험 완료: `{config_file}`")
